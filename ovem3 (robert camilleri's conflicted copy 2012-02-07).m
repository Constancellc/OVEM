...
% total_driven_wheelaxle_inertia=wheel_axle_inertia(motor_in_or_out_of_wheel(i),...
%     number_of_driven_wheels(i),tire_radius(i));
%     
% [max_ice_kw,total_fuel_energy,~,timestep,v_ach,~]=level_3_cv_di_ice(v_req,...
%     t,timestep,drive_cycle_length,slope,Cd(i),Af(i),rho_air,glider_mass(i),...
%     Crr(i),total_driven_wheelaxle_inertia,motor_in_or_out_of_wheel(i),...
...
% fc_kw = max_ice_kw;
% e_motor_peak_T = max_ice_kw*1000/471;
...
[dist_km(i,j),fuel_ice(i,j),fuel_motor(i,j),fuel_fc(i,j),med_ice_eff(i,j),med_motor_eff(i,j),med_fc_eff(i,j),...
    ...
    e_motor_peak_T(i),motor_selection,ESS1_selection,ESS2_selection);
