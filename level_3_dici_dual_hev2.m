function [max_ice_kw,max_motor_kw,fuel_ice,fuel_motor,motor_eff_f,fgb_to_ICE_b_eff,timestep,v_ach,...
    vgb_mass,batt_kwh]=level_3_dici_dual_hev2(powertrain_type,v_req,~,timestep,drive_cycle_length,slope,Cd,Af,rho_air,...
    ...
    batt_V,di_map,ele_map,max_P,peak_ice_T,~,simulate,top_gear_ratio,~,min_n_w,max_n_w,peak_ice_w,gear_ratio_vector,scaling_factor,...
...
NoCells_ESS1_par = round(par_cells);
...
%     [T_b_gb_to_ICE(i),w_b_gb_to_ICE(i),P_b_gb_to_ICE(i),fgb_to_ICE_b_eff(i)]=fgb_backwards_3(T_b_pg1_out(i),...
%         w_b_pg1_out(i),fgb_ICE_ratio,0,0,tire_radius,timestep,motor_in_or_out_of_wheel,T_max_ICE,k_seal,k_bearing,...
...
%     [T_f_gb_to_pg_from_ICE(i),w_f_gb_to_pg_from_ICE(i),P_f_gb_to_pg(i),fgb_to_pg_from_ICE_f_eff(i)]=...
%         fgb_forwards_3(T_f_ICE_to_gb(i),w_f_ICE_to_gb(i),fgb_ICE_ratio,0,0,tire_radius,timestep,motor_in_or_out_of_wheel,...
%         T_max_ICE,k_seal,k_bearing,k_mesh);

%%
%Gb backwards       
[T_b_gb_to_ICE(i),w_b_gb_to_ICE(i),P_b_gb_to_ICE(i),~,~,~,gear_number(i)]= gb_multispeed_backwards(max_P,...
    T_b_pg1_out(i)*w_b_pg1_out(i),T_b_pg1_out(i),w_b_pg1_out(i),peak_ice_T,k_seal,...
...
%ICE
    [w_f_ICE_to_gb(i),T_f_ICE_to_gb(i),ICE_eff(i),volume_of_fuel_used_by_ICE(i)]= ICE_parallel_dual_model(T_b_gb_to_ICE(i),...
        w_b_gb_to_ICE(i),DoD_ESS1(i),DoD_ESS2(i),ESS1_charge_on_limit,...
        ESS2_charge_on_limit,cs_min_speed,cs_min_torque,ICE_speed_vector,...
        ICE_torque_vector,min_w,max_w,ICE_min_torque_w_profile,ICE_min_torque_T_profile,...
        min_P,ICE_max_torque_w_profile,ICE_max_torque_T_profile,di_map,...
...
[~,T_f_gb_to_pg_from_ICE(i),w_f_gb_to_pg_from_ICE(i),~,~,~,gear_number_f(i)]=gb_multispeed_forwards(max_P,...
    P_f_reqd_by_ice(i),T_f_ICE_to_gb(i),w_f_ICE_to_gb(i),peak_ice_T,k_seal,...
    k_bearing,k_mesh,v_ach(i),v_req(i+1),tire_radius,timestep,min_w,peak_ice_w,...
    min_n_w,max_n_w,top_gear_number,top_gear_ratio,gear_ratio_vector,...
    final_drive_ratio,gears/final_drive_ratio,w_range);
%%
    %Planetary gear 2
    [T_b_pg2_out(i),w_b_pg2_out(i)]= planetary_gear_2(T_f_gb_to_pg_from_ICE(i),...
        w_f_gb_to_pg_from_ICE(i),pg_eff(i),T_b_pg1_out(i),w_b_pg1_out(i),...
        ICE_to_motor_GR,pg_to_motor_GR,pg_to_ICE_GR,w_b_wheelaxle_to_pg(i));
...
[T_b_gb_to_motor(i),w_b_gb_to_motor(i),P_b_gb_to_motor(i),fgb_to_motor_b_eff(i)]= fgb_backwards_3(T_b_pg2_out(i),...
    w_b_pg2_out(i),fgb_motor_ratio,0,0,tire_radius,timestep,motor_in_or_out_of_wheel,...
    T_max_motor,k_seal,k_bearing,k_mesh,front_or_rear_wheel_drive); 
        
%Motor backwards
[P_b_motor_to_elecbus(i),w_b_motor_to_elecbus(i),T_b_motor_to_elecbus(i),TperP,motor_eff_b(i)]=motor_backwards_3(T_b_gb_to_motor(i),...
    w_b_gb_to_motor(i),0,0,w_max,rotor_inertia,speed_range,torque_range,...
    eff_matrix,timestep,T_max_speed_vector,T_max_profile,number_of_motors);
...
%     
% figure(4)
% plot(w_f_motor_to_gb*60/(2*pi()),'g')
% hold on
% plot(w_f_ICE_to_gb*60/(2*pi()),'b')
% legend('Motor','ICE')
% ylabel('Speed, RPM')
% hold off
%     
...
% figure(7)
% plot(w_f_ICE_to_gb.*T_f_ICE_to_gb)
% hold on
% plot(P_supp_ESS1,'r')
% hold on
% plot(P_f_bus_to_motor,'g')
...
max_ice_kw = max(P_f_gb_to_pg)/1000;
